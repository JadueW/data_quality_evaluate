    ...
    for signal_group in range(1, number_of_signal_groups + 1):
        ...
        if (signal_group_num_channels > 0) and (signal_group_enabled > 0):
            for signal_channel in range(0, signal_group_num_channels):
                ...
                (
                new_channel['electrode_impedance_magnitude'], new_channel['electrode_impedance_phase']) = struct.unpack(
                    '<ff', fid.read(8))
...
def notch_filter(input, f_sample, f_notch, bandwidth):
    ...
    # Run filter
    for i in range(2, L):
        output[i] = (a * b2 * input[i - 2] + a * b1 * input[i - 1] + a * b0 * input[i] - a2 * output[i - 2] - a1 *
                     output[i - 1]) / a0
...
def read_one_data_block(data, header, indices, fid):
    ...
    if (header['version']['major'] == 1 and header['version']['minor'] >= 2) or (header['version']['major'] > 1):
        data['t_amplifier'][
        indices['amplifier']:(indices['amplifier'] + header['num_samples_per_data_block'])] = np.array(
            struct.unpack('<' + 'i' * header['num_samples_per_data_block'],
    ...
    if header['num_amplifier_channels'] > 0:
        tmp = np.fromfile(fid, dtype='uint16',
        ...
        (indices['amplifier']):(indices['amplifier'] + header['num_samples_per_data_block'])] = tmp.reshape(
            header['num_amplifier_channels'], header['num_samples_per_data_block'])

    if header['num_aux_input_channels'] > 0:
        tmp = np.fromfile(fid, dtype='uint16',
                          count=int((header['num_samples_per_data_block'] / 4) * header['num_aux_input_channels']))
        data['aux_input_data'][range(header['num_aux_input_channels']),
        indices['aux_input']:int(indices['aux_input'] + (header['num_samples_per_data_block'] / 4))] = tmp.reshape(
    ...
    if header['num_supply_voltage_channels'] > 0:
        ...
        data['supply_voltage_data'][range(header['num_supply_voltage_channels']),
        indices['supply_voltage']:(indices['supply_voltage'] + 1)] = tmp.reshape(header['num_supply_voltage_channels'],
                                                                                 1)

    if header['num_temp_sensor_channels'] > 0:
        tmp = np.fromfile(fid, dtype='uint16', count=1 * header['num_temp_sensor_channels'])
        data['temp_sensor_data'][range(header['num_temp_sensor_channels']),
        indices['supply_voltage']:(indices['supply_voltage'] + 1)] = tmp.reshape(header['num_temp_sensor_channels'], 1)
    ...
    if header['num_board_adc_channels'] > 0:
        ...
        indices['board_adc']:(indices['board_adc'] + header['num_samples_per_data_block'])] = tmp.reshape(
            header['num_board_adc_channels'], header['num_samples_per_data_block'])

    if header['num_board_dig_in_channels'] > 0:
        data['board_dig_in_raw'][
        indices['board_dig_in']:(indices['board_dig_in'] + header['num_samples_per_data_block'])] = np.array(
            struct.unpack('<' + 'H' * header['num_samples_per_data_block'],
                          fid.read(2 * header['num_samples_per_data_block'])))

    if header['num_board_dig_out_channels'] > 0:
        data['board_dig_out_raw'][
        indices['board_dig_out']:(indices['board_dig_out'] + header['num_samples_per_data_block'])] = np.array(
            struct.unpack('<' + 'H' * header['num_samples_per_data_block'],
                          fid.read(2 * header['num_samples_per_data_block'])))
...
def load_file(filename):
    ...
    if (data_present):
        ...
        data['amplifier_data'] = np.multiply(0.195,
                                             (data['amplifier_data'].astype(np.int32) - 32768)).astype(np.int32)  # units = microvolts
...
def get_recording_files(filedir):
    finFiles = []
    for dirpath, dirnames, filenames in os.walk(filedir):
        for filename in filenames:
            ...
            if 'whole.rhd' in ff:
                finFiles.append(ff)
    return sorted(finFiles)


def save_data_for_analysis(datafile, for_k=False):
    result, data_present = load_file(datafile)
    if not data_present:
        print('The data file is wrong!')
        return
    else:
        times = result['t_amplifier']
        data_sample_frequency = result['frequency_parameters']['amplifier_sample_rate']
        data = result[
            'amplifier_data']  # notion:new version software saved data with remove 50 hz if notch filter is on

        aux_sample_frequency = result['frequency_parameters']['aux_input_sample_rate']

        if 'board_dig_in_data' in result.keys():
            marker = result['board_dig_in_data']
            data_info = {'time': times,
                         'data_sample_frequency': data_sample_frequency,
                         'aux_sample_frequency': aux_sample_frequency,
                         'marker': marker}
        else:
            data_info = {'time': times,
                         'data_sample_frequency': data_sample_frequency,
        ...
        fileloc_d = os.path.dirname(datafile) + '\\' + dataname + '_data.npy'
        np.save(fileloc_d, data)

        # for kilosorter
        if for_k:
            fileloc_dat = os.path.dirname(datafile) + '\\' + dataname + '_data.dat'
            data.astype('int16').tofile(fileloc_dat)

        fileinfo = os.path.dirname(datafile) + '\\' + dataname + '_info.npy'
        np.save(fileinfo, data_info)
        # np.load(test,allow_pickle=True)


def get_spike_info_file(dirfile):
    parent, _ = os.path.split(dirfile)
    files = os.listdir(parent)
    spike_file = [i for i in files if 'whole_data.npy' in i]
    info_file = [i for i in files if 'whole_info.npy' in i]

    return os.path.join(parent, spike_file[0]), os.path.join(parent, info_file[0])
