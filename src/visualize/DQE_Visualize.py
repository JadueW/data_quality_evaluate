...
raw_dir = '../../results'
compare_g = [dir_name for dir_name in os.listdir(raw_dir) if dir_name.__contains__("对照")]
exper_g = [dir_name for dir_name in os.listdir(raw_dir) if dir_name.__contains__("实验")]
...
def plot_line(compare_json_df, exper_json_df):
    ...
    sns.lineplot(data=compare_json_df, x=compare_json_df.index, y='SNR_Avg',
                 marker='o', label='C', ax=axes[0])  # 现在 axes[0] 是单个子图了
    ...
    # 2.Impedence
    sns.lineplot(data=compare_json_df, x=compare_json_df.index, y='Impedance_Avg',
                 marker='o', label='C', ax=axes[1])
    sns.lineplot(data=exper_json_df, x=exper_json_df.index, y='Impedance_Avg',
    ...
    # 3.STD
    sns.lineplot(data=compare_json_df, x=compare_json_df.index, y='STD_Avg',
                 marker='o', label='C', ax=axes[2])
    sns.lineplot(data=exper_json_df, x=exper_json_df.index, y='STD_Avg',
    ...
    # 4.MEAN
    sns.lineplot(data=compare_json_df, x=compare_json_df.index, y='AMP_Avg',
                 marker='o', label='C', ax=axes[3])
    sns.lineplot(data=exper_json_df, x=exper_json_df.index, y='AMP_Avg',
...
def plot_line_separate(compare_json_df, exper_json_df):
    ...
    for config in plot_configs:
        ...
        sns.lineplot(data=compare_json_df, x=compare_json_df.index, y=config['y_col'],
                     marker='o', label='C', ax=ax)
        sns.lineplot(data=exper_json_df, x=exper_json_df.index, y=config['y_col'],
