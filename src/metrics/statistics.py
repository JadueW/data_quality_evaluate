...
class Statistics:
    ...
    def __init__(self, processed_win_data, compression=100):
        """

        :param processed_win_data: 某个窗口的预处理后的数据
        :param compression: tdigest的聚类数量，用于调节概率分布精度
        """
    ...
    def compute_single_win_statistics(self):
        ...
        for group_id, group_data in self.processed_win_data.items():
            ...
            statistics_win_data = {
                ...
                # TDigest 统计：用于计算百分位数
                "win_tdigest": [TDigest(compression=self.compression) for _ in range(n_channels)],
            }
            ...
            for ch_idx in range(n_channels):
                ...
                # 更新 TDigest 统计
                statistics_win_data['win_tdigest'][ch_idx].update(ch_data)
